...
public class ActuacionDB {
	...
	Boolean add(Actuacion nuevaActuacion){
		...
		gestor.conectar();
		Boolean correcto = gestor.Modificar("INSERT INTO Actuaciones (id_tipoactuacion,id_objetomonitorizable,id_trabajador) VALUES ('"+tipo+"','"+nuevaActuacion.getIdObjMonitorizado()+"','"+nuevaActuacion.getIdTrabajador()+"')");
		gestor.desconectar();
		return correcto;
	}
	...
	ArrayList<Actuacion> getList(String filtro){
		...
		try{
			ResultSet rs;
			gestor.conectar();
			if(Pattern.matches("^d{2}/d{2}/d{2}", filtro)){	// Si el filtrado es por fecha
