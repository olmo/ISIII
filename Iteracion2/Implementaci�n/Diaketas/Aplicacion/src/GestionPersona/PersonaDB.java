...
public class PersonaDB {
	...
	public Boolean consultarPersona(String dni){
		gestor.conectar();
		ResultSet rs=gestor.RealizarConsulta("select * from Personas where dni='"+dni+"'");
	...
	public Boolean darBajaPersona(Integer id){
		...
		gestor.conectar();
		boolean valido=gestor.Modificar("UPDATE Personas SET estado='0',fbaja='"+f+"' WHERE id='"+id+"'");
		gestor.desconectar();
		return valido;
	}
	...
	public int introducirDatos(Persona p){
		gestor.conectar();
		boolean valido=gestor.Modificar("INSERT INTO Personas (dni,nombre, apellido1, apellido2,fnac, telefono,lugarnac,domicilio,cp,email,fbaja) VALUES ('"+p.getDni()+"','"+p.getNombre()+"','"+p.getApellido1()+"','"+p.getApellido2()+"','"+p.getfNacimiento()+"','"+p.getTelefono()+"','"+p.getLugarNacimiento()+"','"+p.getDomicilio()+"','"+p.getCp()+"','"+p.getemail()+"','1900-01-01')");

		int id=-1;
		if(valido){
			ResultSet rs = gestor.RealizarConsulta("select id from Personas where dni='"+p.getDni()+"'");
			try {
				rs.next();
				id=(Integer)rs.getObject("id");
			} catch (SQLException e) {
				JOptionPane.showMessageDialog(null, "Error obtener el id de la persona insertada\n"+e.getMessage());
			}
		}
		gestor.desconectar();
		return id;
	...
	public Persona getDatos(int id){
		...
		Persona p = new Persona();
			...
			p.setemail(rs.getObject("email").toString());
				...
				JOptionPane.showMessageDialog(null, "Error obtener la persona en PersonaDB\n"+e.getMessage());
			}
		gestor.desconectar();
		return p;
	}
	...
	public ArrayList<Persona> buscaPersonas(String persona, String Filtro){
		...
		if(Filtro==null)
			rs = gestor.RealizarConsulta("select * from Personas where nombre LIKE ('"+persona+"%') OR apellido1 LIKE ('"+persona+"%') OR dni LIKE ('%"+persona+"%')");
		else{
			if(Filtro.equals("trabajador"))
				rs = gestor.RealizarConsulta("select * from Personas where ((nombre LIKE ('"+persona+"%') OR apellido1 LIKE ('"+persona+"%') OR dni LIKE ('%"+persona+"%')) AND id IN (select id_persona from Trabajadores))");
			else if(Filtro.equals("beneficiario"))
				rs = gestor.RealizarConsulta("select * from Personas where ((nombre LIKE ('"+persona+"%') OR apellido1 LIKE ('"+persona+"%') OR dni LIKE ('%"+persona+"%')) AND id IN (select id_persona from Beneficiarios))");
			else if(Filtro.equals("donante-Socio"))
				rs = gestor.RealizarConsulta("select * from Personas where ((nombre LIKE ('"+persona+"%') OR apellido1 LIKE ('"+persona+"%') OR dni LIKE ('%"+persona+"%')) AND id IN (select id_persona from Donantes) AND id IN (select id_persona from Socios))");
			else if(Filtro.equals("donante-Empresa"))
				rs = gestor.RealizarConsulta("select * from Personas where ((nombre LIKE ('"+persona+"%') OR apellido1 LIKE ('"+persona+"%') OR dni LIKE ('%"+persona+"%')) AND id IN (select id_persona from Donantes) AND id IN (select id_persona from Empresas))");
			else if(Filtro.equals("donante-Colaborador"))
				rs = gestor.RealizarConsulta("select * from Personas where ((nombre LIKE ('"+persona+"%') OR apellido1 LIKE ('"+persona+"%') OR dni LIKE ('%"+persona+"%')) AND id IN (select id_persona from Donantes) AND id IN (select id_persona from Colaboradores))");
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Error rellenar datos de persona en PersonaDB\n"+e.getMessage());
		}
			i++;
	...
	public boolean darAltaPersona(Integer id) {
		gestor.conectar();
		boolean valido=gestor.Modificar("UPDATE Personas SET estado='1', fbaja='1900-01-01' WHERE id='"+id+"'");
		gestor.desconectar();
		return valido;
